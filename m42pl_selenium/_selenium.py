...
class Selenium:
    ...
    async def setup(self, event, pipeline, context):
        self.hub_url = await self.hub.read(event, pipeline, context)
        self.driver_name = (await self.browser.read(event, pipeline, context)).upper()
        self.session_id = await self.session.read(event, pipeline, context)
        self.driver = None
...
class Get(Selenium, GeneratingCommand):
    ...
    _aliases_   = ['selenium_get', 'selenium_open']
    _schema_    = {
        'properties': {
    ...
    async def target(self, event, pipeline, context):
        try:
            ...
            url = await self.url.read(event, pipeline, context)
            self.driver.get(url)
...
class Find(Selenium, StreamingCommand):
    ...
    def __init__(self, kind, locator, **kwargs):
        """
        :param kind:    Objects kind ('xpath', ...)
        :param locator: Kind's expression (XPath, tag name, ...)
        """
    ...
    async def setup(self, event, pipeline, context):
        ...
        kind_name = await self.kind.read(event, pipeline, context)
        self.kind = getattr(
            By,
    ...
    async def target(self, event, pipeline, context):
        try:
            event = self.get_session(event)
            locator = await self.locator.read(event, pipeline, context)
